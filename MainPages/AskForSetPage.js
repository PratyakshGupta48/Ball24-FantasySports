...
export default function AskForSetPage({navigation}) {
  ...
  const {MatchId,TeamCode1,TeamCode2,Team1,Team2,I1,I2,ContestType,MatchKey,Entry,uid,MatchLink,Free,contestLive,spotsFilled} = useRoute().params;
  ...
  useEffect(() => {if(isPriceModalVisible) LayoutAnimation.configureNext(customLayoutAnimation)}, [isPriceModalVisible]);
  useEffect(() => {
    ...
    const listener2 = firestore().collection('AllMatches').doc(MatchId).collection('4oversContests').doc(MatchKey).onSnapshot(document=>{
      setIsPriceModalFixed((prevIsPriceModalFixed) => {
        if (!prevIsPriceModalFixed && document.data().MaximumSpots - document.data().FilledSpots <= 0)spotsFilled();
        if(!prevIsPriceModalFixed && (document.data().ContestStatus === 'Live' || document.data().ContestStatus === 'Completed'))contestLive();
    ...
    <View style={styles.MainWholeContainer}>
      <TouchableWithoutFeedback onPress={navigateToBallSelection}>
        <View style={styles.UseDefaultContainer} elevation={3}>
          ...
          />}
          <Text style={styles.UseDefaultText}>New Set —{'>'}</Text>
        </View>
      </TouchableWithoutFeedback>
      {setsData &&<View style={styles.UsePreviousMainContainer} elevation={3}>
        <Text style={styles.UseDefaultText}>Select a set</Text>
        <FlatList
          data={setsData}
        ...
        />
        <Text onPress={()=>{if(selectedItemIndices.length>0){setIsPriceModalVisible(true); setIsPriceModalFixed(false);}}} style={selectedItemIndices.length>0 ? [styles.NextButtonContainer,{backgroundColor:'#009e00'}] : styles.NextButtonContainer}>CONFIRM</Text>
      </View>}
      {setsData && <Text style={styles.YouCan}>Note-You can edit the sets later from 'My Contests' section before match deadline passes.</Text>}
    </View>
    {selectedItemIndices.length>0 && setsData && <Modal isVisible={isPriceModalVisible} animationIn={'slideInUp'} animationInTiming={350} animationOut={'slideOutDown'} animationOutTiming={350} backdropOpacity={0.5} onBackdropPress={()=>{setIsPriceModalVisible(isPriceModalFixed)}} hideModalContentWhileAnimating={true}>
      <EntryCalculatorAskForSet AskForSet={selectedItemIndices.map(index=>setsData[index].Name)} MatchId={MatchId} TeamCode1={TeamCode1} TeamCode2={TeamCode2} ContestType={ContestType} MatchKey={MatchKey} Entry={Entry} uid={uid} Free={Free} navigation={()=>{navigation.replace('ContestSelection',{Team1:Team1,Team2:Team2,TeamCode1:TeamCode1,TeamCode2:TeamCode2,MatchId:MatchId,uid:uid,I1:I1,I2:I2,MatchLink:MatchLink})}} navigation2={(ja)=>navigation.navigate('AddCash',{add:'₹'+ja})} modalFix={()=>{setIsPriceModalFixed(true)}} disableModal={()=>{setIsPriceModalVisible(false)}} error={(err,err2)=>{setIsPriceModalVisible(false);showToast('error',err,err2)}} />
    </Modal>}
    <Toast/>
    </>
  )
