...
function ContestSelection({navigation}) {
  ...
  const {Team1,Team2,MatchId,TeamCode1,TeamCode2,I1,I2,uid,CurrentInning} = useRoute().params;
  ...
  {status!='' && status=='Live' && <FetchScore I1={I1} I2={I2} status={status} MatchId={MatchId} Team1={Team1} Team2={Team2} CurrentInning={CurrentInning} page={'ContestSelection'} />}
  ...
  <Tab.Navigator initialRouteName={initialRouteName} optimizationsEnabled={true} screenOptions={{tabBarBounces:true,swipeEnabled:false,tabBarStyle:{position:'relative',height:28,backgroundColor:'#1a1a1a'},lazy:true,lazyPreloadDistance:0,lazyPlaceholder: () => <View style={{ flex: 1, backgroundColor: '#ffffff' }}><SkeletonContestSelection /></View>,tabBarPressColor:'#969696',tabBarIndicatorStyle:styles.tabBarIndicatorStyle}} initialLayout={{width:width}}>
    <Tab.Screen name='Contests' component={ContestSelectionTab} initialParams={{uid:uid,MatchId:MatchId,Team1:Team1,Team2:Team2,TeamCode1:TeamCode1,TeamCode2:TeamCode2,I1:I1,I2:I2,CurrentInning:CurrentInning}} options={{title:({focused})=>(
      <Text style={[styles.contestTabBarTitleStyle,{color: focused ? '#dedede' : '#a1a1a1' }]}>Contests</Text>        
    )}}/>
    <Tab.Screen name='MyContests' component={MyContestsMatchDisplayOnClickLivePage} initialParams={{MatchId:MatchId,Team1:Team1,Team2:Team2,TeamCode1:TeamCode1,TeamCode2:TeamCode2,I1:I1,I2:I2,CurrentInning:CurrentInning}} options={{title:({focused})=>(
