  ...
  <View style={{height:500}}>
    <View style={styles.BallViewMainContainer}>
      ...
      <View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between',width:'100%'}}>
        ...
        {status!=='Live' && <Text style={styles.BallViewNameText}>{TeamCode1+'  v/s  '+TeamCode2}</Text>}
        {status==='Live' && <Text style={[styles.BallViewNameText,{fontSize:14.5,borderBottomWidth:0.6,borderBottomColor:'#dedede',paddingHorizontal:10}]}>{'Points: '+Points+'  ,  Rank: 1'}</Text>}
      ...
      <FlatList
        ...
        renderItem={({item,index}) => (<View style={{flexDirection:'column',alignItems:'center'}}>
          ...
          {status==='Live' && <View style={{flexDirection:'row',alignItems:'baseline',paddingTop:16}}><Text style={styles.PointsText}>{PointsArray[index]!=undefined && PointsArray[index]}</Text><Text style={[styles.PointsText,{fontSize:9,fontFamily:'Poppins-Regular'}]}>{PointsArray[index]!=undefined && ' Pts'}</Text></View>}
        </View>)}
      />
      </View>
    </View>
...
export default React.memo(BallViewLive);
